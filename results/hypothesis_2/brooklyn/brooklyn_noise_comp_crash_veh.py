...
lines = lines.mapPartitions(lambda x: reader(x)).filter(lambda line : line[23].encode('utf-8').strip().upper() == "BROOKLYN")

crash = sc.textFile("/user/sar516/collisions.csv", 1, use_unicode=False)
crash = crash.filter(lambda line: "DATE" not in line)
crash = crash.mapPartitions(lambda x: reader(x)).filter(lambda line : line[2].encode('utf-8').strip().upper() == "BROOKLYN")
...
df_joined.map(lambda x: (x[0], x[1], x[3], 'unexpected' if ((float(x[1]) > ub) or (float(x[1]) < lb)) else 'expected')) \
	...
	.saveAsTextFile("brooklyn_dist_veh.out")
...
sc.parallelize(l) \
	...
	.saveAsTextFile("brooklyn_dist_veh_summary.out")
