...
details = lines.map(lambda line : ("%s\t%s" % (line[48].encode('utf-8').strip(), create_labels(line[48].encode('utf-8').strip()))))
details.saveAsTextFile("49_ferry_terminal_name_summary.out")
sc.stop()
