...
all_lines = sc.textFile("/user/ac5901/complaint_type.csv",1)
...
if len(sys.argv) != 2:
	print("Usage: 6_complaint_type <file>",  file=sys.stderr)
	exit(-1)
lines = sc.textFile(sys.argv[1], 1, use_unicode=False)
lines = lines.mapPartitions(lambda x: reader(x))
details = lines.map(lambda line : ("%s\t%s" % (line[5].encode('utf-8').strip(), create_labels(line[5].encode('utf-8').strip()))))
details.saveAsTextFile("6_complaint_type_summary.out")
sc.stop()
