...
all_lines = sc.textFile("/user/ac5901/garage_lot_name.csv",1)
...
if len(sys.argv) != 2:
	print("Usage: 47_garage_lot_name <file>",  file=sys.stderr)
	exit(-1)
lines = sc.textFile(sys.argv[1], 1, use_unicode=False)
lines = lines.mapPartitions(lambda x: reader(x))
details = lines.map(lambda line : ("%s\t%s" % (line[46].encode('utf-8').strip(), create_labels(line[46].encode('utf-8').strip()))))
details.saveAsTextFile("47_garage_lot_name_summary.out")
sc.stop()
