...
if len(sys.argv) != 2:
	print("Usage: 11_street_name <file>",  file=sys.stderr)
	exit(-1)
lines = sc.textFile(sys.argv[1], 1, use_unicode=False)
lines = lines.mapPartitions(lambda x: reader(x))
details = lines.map(lambda line : ("%s\t%s" % (line[10].encode('utf-8').strip(), create_labels(line[10].encode('utf-8').strip()))))
details.saveAsTextFile("11_street_name_summary.out")
sc.stop()
