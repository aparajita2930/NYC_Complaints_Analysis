...
all_lines = sc.textFile("/user/ac5901/street_name.csv",1)
...
def get_label(val, basetype):
	...
	if basetype == 'TEXT':
		if val is None or len(val.strip()) == 0 or val in ['UNSPECIFIED', 'NA', 'N/A', 'N?A', 'NA/']:
			return 'NULL'
		elif any(ext in val for ext in valid_types):
			return 'VALID'
		else:
			return 'INVALID'
	else:
...
if len(sys.argv) != 2:
	print("Usage: 10_incident_address <file>",  file=sys.stderr)
	exit(-1)
lines = sc.textFile(sys.argv[1], 1, use_unicode=False)
lines = lines.mapPartitions(lambda x: reader(x))
details = lines.map(lambda line : ("%s\t%s" % (line[9].encode('utf-8').strip(), create_labels(line[9].encode('utf-8').strip()))))
details.saveAsTextFile("10_incident_address_summary.out")
sc.stop()
